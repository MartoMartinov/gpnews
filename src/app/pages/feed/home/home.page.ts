      ...
      @if (feed.loading() && feed.articles().length === 0) {
        <div style="padding-top: 4px">
          <gp-skeleton
            width="46%"
            height="22px"
            style="margin: var(--s4) var(--s5) var(--s5); display: block"
          />
        ...
        @for (sec of feed.homeSections(); track sec.cat.id; let i = $index) {
          <div class="gp-catsec">
            <button
              class="gp-section-head as-link"
              (click)="goCategory(sec.cat.id)"
            >
          ...
          @if (i === 0 && !auth.isLoggedIn()) {
            <div class="gp-gate">
              <div class="gate-ico">
                <gp-icon name="shield" [size]="26" [sw]="1.6" />
              </div>
              <div class="gate-txt">
                <b>Регистрирай се за достъп</b>
                <span>до цялото съдържание и известия от бранша.</span>
              </div>
              <gp-btn variant="primary" size="sm" (pressed)="goAuth()"
                >Вход</gp-btn
              >
            </div>
          }
