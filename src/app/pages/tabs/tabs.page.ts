...
/** Bottom tab shell: custom icons + auth-aware tabs matching the design demo. */
@Component({
  selector: 'app-tabs',
        ...
        @if (authStore.isLoggedIn()) {
          ...
          <ion-tab-button tab="notifications">
            <span class="ic-wrap">
              ...
              @if (unreadCount() > 0) {
                <span class="ic-badge">{{ unreadCount() }}</span>
              }
            </span>
          ...
          <ion-tab-button (click)="goLogin()">
            <gp-icon name="user" [size]="23" />
  ...
  styles: [
    `
      /* Active icon gets accent colour; text stays as --color-selected (ink) via Ionic */
      .ic-active {
...
export class TabsPage {
  ...
  /** Wire to NotificationsStore.unreadCount() once it exists. */
